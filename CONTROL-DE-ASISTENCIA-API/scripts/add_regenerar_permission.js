...
async function addPermission() {
    try {
        ...
        // Check if `asistencia.regenerar` exists
        const exists = result.recordset.some(p => p.NombrePermiso === 'asistencia.regenerar');

        if (!exists) {
            console.log('Adding "asistencia.regenerar"...');
            // Insert it
            const insertRes = await pool.request().query(`
                ...
                VALUES (@NewId, 'asistencia.regenerar', 'Capacidad de regenerar fichas de asistencia desde la auditoria', 1);

                INSERT INTO RolesPermisos (RoleId, PermisoId) VALUES (1, @NewId);
            `);
            console.log('Inserted and assigned to admin (RoleId=1).');
        } else {
            console.log('Permission "asistencia.regenerar" already exists.');
        }
