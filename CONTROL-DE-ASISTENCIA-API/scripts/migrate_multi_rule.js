...
const dbConfig = {
    user: 'sa', // Assuming standard local dev or integrated security
    password: 'root', // Need to pull from env ideally but let's try reading config
    server: 'localhost',
    database: 'BMS_CA',
    options: {
        encrypt: false,
...
// Instead of hardcoding, let's just require the db config from the project
const { dbConfig: realConfig } = require('../dist/config/database');

async function run() {
    try {
        console.log('Connecting to database...');
        const pool = await sql.connect(realConfig);
