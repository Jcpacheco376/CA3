...
CREATE PROCEDURE [dbo].[sp_Reporte_Prenomina]
    ...
    @UsuarioId INT,
    @GrupoNominaId INT,
    @Regenerar BIT = 0  -- 1 = forzar regeneración (borra existente y recalcula), 0 = comportamiento original
AS
BEGIN
    SET NOCOUNT ON;

    -- ========================================================================
    -- 1. Si se solicita regeneración, borrar el reporte existente de forma segura
    -- ========================================================================
    IF @Regenerar = 1
    BEGIN
        BEGIN TRANSACTION;
    ...
    BEGIN
        ...
        BEGIN TRY
            ...
            USING (
                SELECT DISTINCT EmpleadoId, DepartamentoId, PuestoId
                FROM #CalculoTemp
            ) AS Source ON 1 = 0
    ...
    SELECT
        ...
        (
            SELECT
                det.Fecha,
                c.CodRef AS ConceptoCodigo,      -- Puedes cambiar a c.Abreviatura cuando lo uses
