...
BEGIN
    ...
    IF @IdPeriodo IS NULL
    BEGIN
        RAISERROR('ERROR DE DEBUG: No se encontró el periodo en la tabla [ctA].[dbo].[nom10002] para las fechas dadas.', 16, 1);
        RETURN;
    END

    -- ------------------------------------------------------------------------------------------
    -- 2. INSERCIÓN MASIVA EN TABLA DE MOVIMIENTOS (nom10010)
    -- ------------------------------------------------------------------------------------------
