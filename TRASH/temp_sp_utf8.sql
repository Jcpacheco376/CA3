...
BEGIN
    SET NOCOUNT ON;
    SET DATEFIRST 1; -- Lunes es el primer día de la semana
    ...
    -------------------------------------------------------------------
    -- 4. CÁLCULO CON ARITMÉTICA SEGURA (DATEADD)
    -------------------------------------------------------------------
    ;WITH 
    FechasDelRango AS (
    ...
    -- B. Ventanas Calculadas (CORRECCIÓN CRÍTICA AQUÍ)
    VentanasCalculadas AS (
        SELECT
            ...
            -- CORRECCIÓN 1: Usar DATEADD en lugar de CAST + CAST para Entrada
            ...
            -- CORRECCIÓN 2: Usar DATEADD para Salida
            CASE WHEN hd.HoraSalida IS NOT NULL THEN
                    CASE WHEN hd.HoraSalida < hd.HoraEntrada 
                         -- Si cruza medianoche, sumamos 1 día a la Fecha base
    ...
    PreMerge AS (
        SELECT 
            ...
            CASE 
                ...
                -- Si la ventana ya cerró, no podemos dejarlo en proceso, debemos cerrarlo como BORRADOR (o VALIDADO si quisieras auto-aprobar)
                -- Aquí mantengo BORRADOR para que se revise manualmente si quedó incompleto.
