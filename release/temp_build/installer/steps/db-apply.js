...
async function applyDatabase(cfg, analysis, installerDir, log, onProgress) {
    ...
    try {
        ...
        pool.on('error', err => {
            log(`⚠️ Error crítico en el Pool de BD: ${err.message}`);
        });
        ...
        if (errors.length > 0) {
            const logDir = path.join(process.cwd(), 'logs_instalador');
            if (!fs.existsSync(logDir)) fs.mkdirSync(logDir, { recursive: true });
            const logFile = path.join(logDir, `db-errors-${Date.now()}.log`);
            fs.writeFileSync(logFile, errors.join('\n'), 'utf8');
            log(`⚠️  Hubo ${errors.length} error(es) menores. Revisa: ${logFile}`);
        }
...
async function execScript(pool, sqlText, cfg, filePath) {
    ...
    if (_sqlcmd && cfg && sqlText) { // We don't need filePath anymore
        // Escribir el SQL a un archivo temporal para que sqlcmd lo lea directamente
        const os = require('os');
        const tmpFile = path.join(os.tmpdir(), `ca3_tmp_${Date.now()}_${Math.floor(Math.random() * 1000)}.sql`);
        try {
