...
async function installFiles(appConfig, installerDir, log, onProgress) {
    ...
    // ── PASO 1: Crear estructura de destino y Limpiar instalaciones previas ──
    log(`Carpeta de instalación: ${installBase}`);

    // Regla: Borrar el segmento final (api-asistencia / frontend-asistencia) si ya existen
    // para garantizar una instalación limpia y sin archivos residuales.
    try {
        if (fs.existsSync(destApiDir)) fs.rmSync(destApiDir, { recursive: true, force: true });
        if (fs.existsSync(destFrontDir)) fs.rmSync(destFrontDir, { recursive: true, force: true });
    } catch (e) {
        log(`⚠️  No se pudo limpiar completamente la instalación anterior: ${e.message}`);
    }
    ...
    if (fs.existsSync(srcApiDir)) {
        ...
        log('Instalando dependencias del servidor (puede tardar un momento)...');
        try {
            await execAsync('npm install --omit=dev', { cwd: destApiDir });
            log('✅ Dependencias del servidor instaladas.');
        } catch (err) {
            log(`⚠️  Error instalando dependencias: ${err.message}. Deberá ejecutar 'npm install' manualmente en la carpeta api-asistencia.`);
        }
    } else {
