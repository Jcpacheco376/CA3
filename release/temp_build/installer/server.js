...
process.on('uncaughtException', (err) => {
    try {
        const crashLog = path.join(os.tmpdir(), `ca3_installer_crash_${Date.now()}.log`);
        fs.writeFileSync(crashLog, err.stack || err.message || String(err));
        console.error('🔥 CRASH DETECTED. Log saved to: ' + crashLog);
    } catch (_) { }
    process.exit(1);
});
    ...
    try {
        ...
        if (endpoint === '/api/install' && req.method === 'POST') {
            ...
            setImmediate(async () => {
                try {
                    ...
                    await installFiles(appConfig, __dirname, log, (p) => { installState.progress = 40 + p * 0.4; });
                    ...
                    // Escribir log de error asegurando que no intentamos escribir en una carpeta de solo lectura de pkg
                    const logDir = path.join(__dirname, '..', 'logs_instalador');
        ...
        if (endpoint === '/api/finish' && req.method === 'POST') {
            ...
            try {
                ...
                const vFile = path.join(__dirname, '..', 'database', 'version.json'); // En PKG siempre está en __dirname/../
                if (fs.existsSync(vFile)) {
