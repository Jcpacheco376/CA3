...
(function () {
    ...
    function goTo(n) {
        ...
        for (let i = 0; i < n; i++) document.querySelector(`#step-${i}`)?.classList.add('done');
        for (let i = n + 1; i < TOTAL_SCREENS; i++) {
            const el = document.querySelector(`#step-${i}`);
            if (el) { el.classList.remove('done'); el.classList.remove('active'); }
        }

        const meta = SCREEN_META[n];
        if (meta) {
        ...
        if (n === 3) runAnalysis();
        if (n === 7) {
            // Al entrar al paso 8 (Listo), iniciar el launcher en background
            api('POST', '/api/start-launcher');
        }
    ...
    async function handleNext() {
        switch (currentScreen) {
            ...
            case 6: goTo(7); break; // Transición de "Instalando" (100%) a "Listo"
            case 7:
    ...
    async function handleFinalize() {
        ...
        // Abrir el sistema en el navegador predeterminado
        const port = appConfig.API_PORT || 3001;
        const url = `http://localhost:${port}`;
        await api('POST', '/api/open-url', { url });

        // Finalizar el servidor instalador y cerrar UI
    ...
    function startPolling() {
        ...
        pollTimer = setInterval(async () => {
            ...
            if (state.step === 'done') {
                ...
                // NO avanzar automáticamente. Habilitar botón Siguiente.
                btnNext.textContent = 'Siguiente';
                btnNext.disabled = false;
                // No sobre-escribimos onclick aquí porque 'handleNext' ya tiene el caso 6
