...
import { Loader2, ClipboardCheck, Briefcase, Building, Cake, GripVertical, Contact, InfoIcon as Info } from 'lucide-react';
...
export const SchedulePage = () => {
    ...
    const [dragInfo, setDragInfo] = useState<{ employeeId: string; dayIndex: number; scheduleId: string | null } | null>(null);
    const [draggedCells, setDraggedCells] = useState<string[]>([]);
    const [openCellId, setOpenCellId] = useState<string | null>(null);
    
    const [selectedDepartment, setSelectedDepartment] = useState(() => user?.Departamentos?.length === 1 ? user.Departamentos[0].departamento : 'all');
    const [selectedPayrollGroup, setSelectedPayrollGroup] = useState(() => user?.GruposNomina?.length === 1 ? user.GruposNomina[0].grupo_nomina : 'all');
    ...
    const handleBulkScheduleChange = async (updates: { empleadoId: string, fecha: Date, horarioId: string | null }[]) => {
        if (!canAssign) {
        ...
        setEmployees(prev => {
            ...
            updates.forEach(({ empleadoId, fecha, horarioId }) => {
                const empIndex = newEmployees.findIndex((e: any) => e.empleado === empleadoId);
                if (empIndex > -1) {
    ...
    const handleDragStart = (employeeId: string, dayIndex: number, scheduleId: string | null) => {
        if (!canAssign) return;
        setOpenCellId(null);
                        ...
                        {filteredEmployees.map((emp) => (
                            <tr key={emp.empleado} className="group">
                                <td className="p-2 text-left sticky left-0 bg-white group-hover:bg-slate-50 z-10 shadow-sm align-top">
                                    <div className="w-full flex flex-col justify-center" style={{ minHeight: '4rem', minWidth: `${EMPLOYEE_CONTENT_MIN_WIDTH}px`, maxWidth: `${EMPLOYEE_CONTENT_MAX_WIDTH}px` }}>
                                ...
                                {dateRange.map((day, dayIndex) => {
                                    ...
                                    return (
                                        <ScheduleCell
                                            ...
                                            canAssign={canAssign}
                                            onDragStart={(scheduleId: string | null) => handleDragStart(emp.empleado, dayIndex, scheduleId)}
    ...
    if (!canRead) {
        return <div className="p-8 text-center text-red-600 bg-red-50 rounded-lg">No tienes permiso para acceder a esta funcionalidad. Contacta a un administrador.</div>;
    }
