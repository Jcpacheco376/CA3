...
import {
    ...
    ThumbsUp, ThumbsDown, ArrowRight, UserCheck, ChevronDown, Search, Check
} from 'lucide-react';
...
useEffect(() => {
    ...
    // --- HELPERS DE FECHAS CORREGIDOS (FIX TIMEZONE) ---
    // Esta función anula el desfase horario del navegador
                ...
                <div className="w-2/5 flex flex-col border-r border-slate-200 bg-white pr-0">
                    ...
                    <div className="p-5 pb-4">
                        <div className="flex justify-between items-start mb-2">
                            ...
                            <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${header.Estado === 'PorAutorizar' ? 'bg-amber-50 text-amber-600 border-amber-100' :
                        ...
                        <div className="flex flex-wrap gap-3 mt-3 text-xs text-slate-500 items-center">
                            ...
                            <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-md border ${header.AsignadoA ? 'bg-indigo-50 border-indigo-100 text-indigo-700' : 'bg-slate-100 border-slate-200 text-slate-500'}`}>
                                <UserCheck size={12} />
                                <span className="font-semibold">
                                    {header.AsignadoA ? `Asignado a: ${header.AsignadoA}` : 'Sin asignar'}
                                </span>
                            </div>
                        </div>
                    </div>
                                ...
                                <div key={log.BitacoraId} className="relative flex gap-3 group">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-baseline justify-between mb-1 pl-1">
                                            ...
                                            {/* FECHA Y HORA RELATIVA CORREGIDA */}
                                        ...
                                        <div className={`p-3 rounded-xl rounded-tl-none shadow-sm border text-sm leading-relaxed ${log.Accion === 'Resolver' ? 'bg-emerald-50 border-emerald-100 text-emerald-900' :
                                            log.Accion === 'SolicitarAutorizacion' ? 'bg-amber-50 border-amber-100 text-amber-900' :
                                                log.Accion === 'Asignar' ? 'bg-blue-50 border-blue-100 text-blue-900' :
