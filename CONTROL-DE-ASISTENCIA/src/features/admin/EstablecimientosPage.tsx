// src/features/admin/EstablecimientosPage.tsx
...
export const EstablecimientosPage = () => {
    ...
    const canManage = can('catalogo.establecimientos.manage');
    const canRead = can('catalogo.establecimientos.read');
    ...
    const fetchData = useCallback(async () => {
        ...
        setError(null);
        try {
            const res = await fetch(`${API_BASE_URL}/catalogs/establecimientos/management`, { headers });
            if (!res.ok) {
    ...
    const filteredData = useMemo(() => {
        ...
        return data.filter(item =>
            ...
            (item.EstablecimientoId?.toString().toLowerCase().includes(lowercasedFilter))
        );
    }, [data, searchTerm]);

    const renderContent = () => {
        if (isLoading) return <div className="text-center p-8 flex justify-center items-center"><Loader2 className="animate-spin mr-2"/>Cargando establecimientos...</div>;
