...
export const DepartamentosPage = () => {
    ...
    const handleSave = async (item: any) => {
        ...
        try {
            await fetch(`${API_BASE_URL}/api/departamentos`, {
                method: 'POST',
                headers: { 
    ...
    if (isLoading) return <div className="text-center p-8">Cargando departamentos...</div>;
        ...
        <div>
            <header className="flex justify-between items-center mb-6">
                 <div className="flex items-center space-x-3">
                    ...
                    <Tooltip text="Lista de departamentos registrados en el sistema de origen.">
                        <span><InfoIcon /></span>
                    </Tooltip>
                </div>
                {can('catalog.manage') && (
            ...
            <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        {data.map(item => (
                            <tr key={item.departamento} className="border-t border-slate-200">
                                <td className="p-3 font-mono text-slate-500">{item.departamento.trim()}</td>
